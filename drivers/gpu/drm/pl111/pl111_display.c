			       ...
			       struct drm_crtc_state *cstate)
{
	...
	if (fb) {
		u32 offset = pl111_fb_cma_get_gem_addr(fb, pstate, 0);
		 ...
		 * controls it.
		 */
		if (fb->pitches[0] != mode->hdisplay * pl111_format_plane_cpp(fb->pixel_format, 0))
			return -EINVAL;
				 ...
				 struct drm_plane_state *old_pstate)
{
	...
	if (fb) {
		u32 addr = pl111_fb_cma_get_gem_addr(fb, pstate, 0);
