...
#include <drm/bridge/baikal_dw_hdmi.h>
		...
		const struct drm_display_mode *mode)
{
	...
	/* After each CLKDIS reset it is mandatory to
	   set up VSYNC active edge delay (in lines) */
	hdmi_writeb(hdmi, mode->vsync_end - mode->vsync_start, HDMI_FC_VSYNCINWIDTH);
...
static int dw_hdmi_setup(struct dw_hdmi *hdmi, struct drm_display_mode *mode)
{
	...
	dw_hdmi_enable_video_path(hdmi, mode);
		 ...
		 const struct dw_hdmi_plat_data *plat_data)
{
	...
	ret = drm_bridge_attach(encoder->dev, &hdmi->bridge);
	if (ret < 0) {
		hdmi->bridge.dev = NULL;
		hdmi->bridge.encoder = NULL;
	} else
		hdmi->bridge.encoder = encoder;
