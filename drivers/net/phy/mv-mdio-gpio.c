...
// phase.
int mv_mdiobb_cmd_addr(struct mdiobb_ctrl *ctrl, int phy, u32 addr)
{
...
static void mv_mdio_init_clk(struct mv_mdio_data *data) {
  if (!data) {
    ...
    printk (KERN_INFO "%s   data is NOT NULL     ---------\n",__func__);
  }
  return;
}
...
mv_mdio_gpio_probe(struct platform_device *pdev)
{
  ...
  if (pdev)
  printk (KERN_INFO "%s   pdev is NOT NULL     ---------\n",__func__);
  else
  printk (KERN_INFO "%s   pdev is NULL     ---------\n",__func__);
  ...
  if (!dn) {
    ...
    printk (KERN_INFO "%s   dn is NOT NULL\n",__func__);
  }
